...
const validateIngredientId = async (req, res, next) => {
	try {
		...
		if (!uuidValidate(ingredientId)) {
			return res
				.status(400)
				.json({ error: "ID do ingrediente inválido" });
		}

		const [registeredDish] = await dishRepository.getDishById(dishId);
		const [registeredIngredient] =
			await ingredientRepository.findIngredientById(ingredientId);

		if (!registeredIngredient) {
			return res
				.status(404)
				.json({ error: "Ingrediente não encontrado" });
		}

		if (registeredDish.id !== registeredIngredient.dish_id) {
			return res
				.status(404)
				.json({ error: "ID do prato e/ou ID do ingrediente inválido" });
		}
